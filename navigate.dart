              ...
              child: ElevatedButton(
                onPressed: () {
                  if (_formkey.currentState!.validate()) {
                    ...
                    input.add(myController.text);
                    input.add(myIdController.text);
                    input.add(myEmailController.text);
                    // showDialog(
                    //   context: context, 
                    ...
                    Navigator.push(
                      context,
                      MaterialPageRoute(builder: (context) => ShowInput(input: input)),
                    );
                    dispose();
                  }
                },
...
class ShowInput extends StatelessWidget {
  const ShowInput({super.key, required this.input});
  final List<String> input;
    ...
    return Scaffold(
      ...
      body: Column(
        ...
        children: <Widget>[
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 30, vertical: 8),
            child: Text('Name: ${input[0]}'),
          ),
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 30, vertical: 8),
            child: Text('Student ID: ${input[1]}'),
          ),
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 30, vertical: 8),
            child: Text('Email: ${input[2]}'),
          ),
        ],  
      ),
    );
